...
public class Main_gcp {
	...
	/**
	 * Ein Durchlauf des Algorithmus_2. Die maximale Ähnlichkeit der
	 * Ausgangspopulation liegt bei 100% und muss bei Bedarf im Quelltext geändert
	 * werden.
	 * 
	 ...
	 * @param crossover_amount Anteil der aus Rekombination hervorgegangen
	 *                         Individuen
	 * @param q                Anzahl der Gegner für die Turnierselektion
	 * @param mutpr            Mutationswahrscheinlichkeit
	 */
	public static void gcp_test(String tn, String db, int popsz, int max_gen, double selection_amount,
			double crossover_amount, int q, double mutpr) {
		setID(0);
		setDatabase(db);
		setTablename(tn);
		setPopsize(popsz);
		...
		while (generation < max_gen && fittest[0].getFitness() > 0) {
			...
			for (int i = 0; i < getPopsize() * selection_amount; i++) {
				 ...
				 * System.out.println("champ:      "+champ.getFitness()); int[] genome_copy =
				 * new int[N*N]; for(int g = 0; g < (N*N); g++){ genome_copy[g] =
				 * champ.getGenome()[g]; } selected_individuals[i] = new
				 * Individual_2(genome_copy);
				 */
		...
		for (int i = 1; i < popsize; i++) {
			while (!divers) {
				...
				for (int j = i - 1; j >= 0; j--) {
					...
					if (Diversity_2.calc_similarity(initial_pop[i].getGenome(),
							initial_pop[j].getGenome()) > max_simil) {
						divers = false;
						...
						// for-Schleife wird vorzeitig abgebrochen, damit ein neues Individual_2
	 ...
	 * Ähnlichkeitszuordnung, der Ähnlichkeitswert und die ID-Nummer hinzugefügt.
	 * 
	 * @param database   Name der Datenbank
	 * @param table_name Name der Tabelle
	...
	public static void setInitial_pop(Individual_2[] initial_pop) {
		Main_gcp.initial_pop = initial_pop;
	}
