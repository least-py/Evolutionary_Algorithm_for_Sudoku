...
/**
 * Diese Klasse dient der Gewinnung von Ähnlichkeitsdaten, um Aussagen über die
 * Diversität zu treffen.
 * 
 * @author Lea
 *
 */
public class Diversity_2 {
	...
	public static double calc_similarity(int[] genome_1, int[] genome_2) {
		int match = 0;
		for (int g = 0; g < (Main_gcp.getN() * Main_gcp.getN()); g++) {
			if (genome_1[g] == genome_2[g]) {
				match++;
			}
		}
		return (double) match / (Main_gcp.getN() * Main_gcp.getN());
	}

	/**
	 * Ordnet einem Individuum ein Ausgangsindividuum mit der höchsten
	 * Übereinstimmung zu.
	 * 
	 * @param i Individuum
	 * @return den Index des zu dem Individuum ähnlichen Ausgangsindividuums
	 *         innerhalb des Ausgangspopulationsarrays
	 */
	public static void associate_similarity(Individual_2 i) {
		...
		for (Individual_2 ii : Main_gcp.getInitial_pop()) {
