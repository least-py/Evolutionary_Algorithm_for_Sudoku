...
public class Main_1 {
	...
	/**
	 * Ermöglicht einen Durchlauf der Evolutionsstrategie. Standardmäßig ist eine Populationsgröße von
	 * 50 Individuen gewählt. Der maximale Ähnlichkeitsanteil der Ursprungspopulation liegt bei 100% und muss innerhalb
	 * des Quellcodes verändert werden, womit man eine höhere Laufzeit erwarten darf.
	 ...
	 * @param total_swaps Maximal mögliche Mutationen pro Individuum einer Generation
	...
	public static void ev_stragety(String tn, String db, double selection_amount, double crossover_amount, int total_swaps, int max_gen, double m, double co) {
		setID(0);
		setDatabase(db);
		setTablename(tn);
		setPopsize(50);
		...
		while(generation < max_gen && fittest[0].getFitness() > 0) {
			...
			//Wird nur die Referenz des Individuums kopiert, wird bei der Mutation eines dieser Individuum
			...
			//Der "hintere" Teil der Population, der im Folgenden kopiert wird, enthält größtenteils 
	 ...
	 * @param co Rekombinationswahrscheinlichkeit
	 */
	public static void exp(int exp, String tn, String db, double selection_amount, double crossover_amount, int total_swaps, int max_gen, double m, double co) {
		setID(0);
		setDatabase(db);
		setTablename(tn);
		setPopsize(25);
		...
		for(int i = 1; i < getPopsize(); i++) {
			while(!divers) {
				...
				for(int j = i-1; j >=0; j--) {
					...
					if( Diversity_1.calc_similarity(getInitial_pop()[i].getGenome(), getInitial_pop()[j].getGenome()) > max_simil) {
						divers = false;
						...
						//for-Schleife wird vorzeitig abgebrochen, damit ein neues Individual_1 generiert werden kann
						break;
					}
				}
			}
	 ...
	 * die Ähnlichkeitszuordnung, der Ähnlichkeitsanteil und die ID-Nummer hinzugefügt.
	 * @param database Name der Datenbank
	 * @param table_name Name der Tabelle
