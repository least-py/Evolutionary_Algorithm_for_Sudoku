...
public class Evolution_2 {
	...
	/**
	 * Die Methode selektiert Individuen, indem sie q Individuen zufällig gleichverteilt auswählt 
	 * und das beste (mit der besten Fitness) anschließend ausgibt. Je höher q gewählt wird, desto höher ist der
	 * Selektionsdruck.
	...
	/**
	 * Bestimmt zufällig ein Individuum der Population. Alle Individuen haben die selbe Chance.
	 * @param pop die Population
	...
	/**
	 * Dieser Rekombinationsoperator vermischt die Genome der beiden gegebenen Individuen. Bei jedem Gen 
	 * entscheidet die angegebene Wahrscheinlichkeit, ob das Gen des besseren Individuums ausgewählt wird.
	 * @param iv_1
	 * @param iv_2
	 * @param cprob
	...
	public static Individual_2 parameterized_uniform_crossover(Individual_2 iv_1, Individual_2 iv_2, double cprob) {
		int N_squared = Main_2.getN()*Main_2.getN();
	...
	private static int[][] adjacency_array(){
		...
		int[][] index_grid = new int[Main_2.getN()][Main_2.getN()];
		int index = 0;
		for(int z = 0; z < Main_2.getN(); z++) {
			for(int s = 0; s < Main_2.getN(); s++) {
				index_grid[z][s] = index;
				index++;
			}
		}

		int[][] box_squares = new int[Main_2.getN()][Main_2.getN()];
		index = 0;
		//Box beginnt links oben, nächste Box ist die Box darunter
		int sqroot = (int)Math.sqrt(Main_2.getN());
		int[] box = new int[Main_2.getN()];
		//hb steht für horizontale box, damit die vertikale Erstellabfolge horizontal verschoben werden kann
		for(int hb = 0; hb < sqroot; hb++) {
			//vb steht für vertikale box, damit die vertikale Erstellabfolge gewährleistet wird
			for(int vb = 0; vb < sqroot; vb++) {
						
				//Box erstellen: Zeilen werden bis zum "Boxrand" dem boxarray nacheinander hinzugefügt
				for(int z = 0; z < sqroot; z++) {
					for(int s = 0; s <  sqroot ; s++) {
						//(z*sqroot)+s sorgt dafür, dass die Elemente den richtigen Indices von 0 bis n-1 zugeordnet werden
		...
		//Durch das Hinzufügen der jeweilig ganzen Zeile, Spalte und Box werden manche Indizes doppelt oder dreifach aufgenommen
		//Die spätere Nutzung der Indexlist beeinflusst dies jedoch nicht (außer Schleifen)
		//Ist es aufwendiger die Häufungen zu entfernen, als mehrfach abzufragen?
		int[][]adjacency_array = new int[N_squared][3*Main_2.getN()];
		//i repräsentiert einen Knoten
		for(int i = 0; i < N_squared; i++) {
			//leichteres Hinzufügen von Elementen
			ArrayList<Integer> list= new ArrayList<Integer>();
			//Box hinzufügen
			for(int b = 0; b < Main_2.getN(); b++) {
				for (int n = 0; n < Main_2.getN(); n++) {
					if( i == box_squares[b][n]) {
						for(int e = 0; e < Main_2.getN(); e++) {
			...
			//Spalte hinzufügen
			for(int e = 0; e < Main_2.getN(); e++) {
				list.add((i%Main_2.getN())+(e*Main_2.getN()));
			}
			//Zeile hinzufpgen
			int index_row = (int)(i/Main_2.getN());
			for(int e = 0; e < Main_2.getN(); e++) {
				list.add(index_grid[index_row][e]);
			}
			//Das Array an der Stelle i repräsentiert die Knoten, die mit dem Knoten i verbunden sind (!Auf Schleife achten!)
			for(int n = 0; n < 3*Main_2.getN(); n++) {
				adjacency_array[i][n] = list.get(n);
			}
		}
		return adjacency_array;
	}
	//nicht unnötig? Ohne die Matrix werden die Schleifen und das Abfragen von bereits abgefragten Verbindungen verhindert
	
	/**
	 * Erzeugt die Adjazenzmatrix für ein Sudoku bestimmten Grades. Der Grad wird durch den Parameter in Main_2 bestimmt.
	 * 
	...
	public static int[][] adjacency_matrix() {
		...
		for(int i = 0; i < N_squared; i++) {
			for( int e : adjacency_array[i]) {
				//Da diese adjacency_matrix schleifenfrei ist (Kein Knoten ist durch eine Kante mit sich selbst verbunden), muss die Hauptdiagonale aus Nullen bestehen
				if( e != i) {
					adjm[i][e] = 1;
				}
			}
	...
	public static void print_matrix(int[][] matrix) {
		...
		/**
		 * Mit dieser Methode lässt sich eine bestimmte Anzahl an Individuen mit der besten Fitness auswählen.
		...
		/**
		 * Ermittelt und setzt die Fitness eines Individuums mit Hilfe der Adjazenzmatrix. N muss in Main_2 gesetzt sein.
		 * @param iv Individuum
		 */
		...
		/**
		 * Mutiert ein Individuum, indem die Knoten mit einer Farbe eingefärbt werden, die vorher noch nicht ermittelt wurde.
		 * @param iv Individuum
		 */
		...
		*/
		...
		/**
		 * Mutiert ein Indiviuum, indem der Knoten mit einer zufälligen Farbe neu eingefärbt wird.
		 * @param iv Individuum
		 */
